...
from file_columns import (
    REPORT_STAYS_COLUMNS,
)
...
class TestValidateFileHasAllTheColumns:
    ...
    def test_success_when_all_columns_are_present(self, excel_df):
        # Should **not** raise
        validate_file_has_all_the_columns(excel_df, REPORT_STAYS_COLUMNS)

    def test_raise_when_a_column_is_missing(self, excel_df):
        missing_col = REPORT_STAYS_COLUMNS.get_all_excel_columns()[-1]
        df_without_column = excel_df.drop(columns=[missing_col])

        with pytest.raises(Exception) as exc_info:
            validate_file_has_all_the_columns(df_without_column, REPORT_STAYS_COLUMNS)
...
class TestValidateFilteredReportStay:
    ...
    def test_success_with_valid_dataframe(self, valid_report_stay_df):
        # Should **not** raise
        validate_filtered_report_stay(valid_report_stay_df, REPORT_STAYS_COLUMNS)
    ...
    def test_raise_when_header_is_missing_column(self, valid_report_stay_df):
        missing_col = REPORT_STAYS_COLUMNS.get_renamed_columns_to_use()[0]
        df_without_column = valid_report_stay_df.drop(columns=[missing_col])

        with pytest.raises(Exception):
            validate_filtered_report_stay(df_without_column, REPORT_STAYS_COLUMNS)
