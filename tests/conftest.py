...
def valid_report_stay_df() -> pd.DataFrame:
    ...
    renamed_cols = get_renamed_columns_to_use()

    numeric_default = {c: 100.0 for c in get_numeric_columns()}
    datetime_default = {c: "01/01/2023" for c in get_datetime_columns()}
    string_default = {c: "test" for c in get_string_columns()}
